...
const tag = tv({
  base: 'h-6 transition transform relative inline-flex items-center rounded-lg overflow-hidden border gap-1',
  variants: {
    color: {
      ...
      warning: 'bg-yellow-500/10 dark:bg-yellow-500/20 text-yellow-700 dark:text-yellow-300 border-yellow-500/20',
    ...
    size: {
        default: 'h-6 text-sm px-1.5 font-medium',
...
export const Tag = ({ children, color, title, type, size }: { children: React.ReactNode; color?: 'default' | 'error' | 'success' | 'warning'; title?: string; type?: 'default' | 'link'; size?: 'default' | 'sm' }) => {
    return(
        <span
...
export const TagLink = ({ href, children, color, title, type, size }: { href: string; children: React.ReactNode; color?: 'default' | 'error' | 'success' | 'warning'; title?: string, type?: 'default' | 'link'; size?: 'default' | 'sm' }) => {
    return(
